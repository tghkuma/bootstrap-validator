export class BootstrapValidator {
  ...
  displayError(arrErrors) {
    arrErrors.forEach(error => {
      this.setError.apply(this, [error.name, error.messages]);
    });
    if (0 < arrErrors.length) {
      // 最初のエラーにフォーカス
      this.focusError.apply(this, [arrErrors[0].name]);
    }
  }
  ...
  setError(name, messages) {
    if (typeof this.settings.setError === 'function') {
      ...
      this.setErrorBootstrap.apply(this, [name, messages]);
    }
  }
  ...
  clearError(name) {
    if (typeof this.settings.clearError === 'function') {
      this.settings.clearError.apply(this, [name]);
      // } else if (this.settings.errorType === 'bs5') {
      //   this.clearErrorBootstrap5.apply(this, [name]);
    } else {
      this.clearErrorBootstrap.apply(this, [name]);
    }
  }
  ...
  _validExistsFunc = class {
    ...
    static email(field, nodeList, params, v) {
      ...
      if (val) {
        const email_error = v._helpers.isEmailEx.apply(v, [val]);
        if (email_error !== '') {
          return email_error;
        }
      }
    ...
    static minlength(field, nodeList, params, v) {
      ...
      if (v._helpers.getValue(nodeList).length < min)
        return v._helpers.format(v.settings.messages.MIN_LENGTH, min);
      return null;
    }
    /**
     * 最大文字数
    ...
    static maxlength(field, nodeList, params, v) {
      ...
      if (max < v._helpers.getValue(nodeList).length)
        return v._helpers.format(v.settings.messages.MAX_LENGTH, max);
      return null;
    }
    /**
     * 数値桁数チェック
    ...
    static numlength(field, nodeList, params, v) {
      ...
      if (!reg.test(val)) {
        return v._helpers.format(v.settings.messages.NUM_LENGTH, err_message_tmp);
      }
      return null;
    }
    /**
    ...
    static min(field, nodeList, params, v) {
      ...
      const min = Number(params[0]);
      if (val < min)
        return v._helpers.format(v.settings.messages.MIN, min);
      return null;
    }
    /**
     * 最大値
    ...
    static max(field, nodeList, params, v) {
      ...
      const max = Number(params[0]);
      if (max < val)
        return v._helpers.format(v.settings.messages.MIN, max);
      return null;
    }
    /**
     * 数値範囲
    ...
    static range(field, nodeList, params, v) {
      ...
      const min = Number(params[0]),
        max = Number(params[1]);
      if (val < min || max < val)
        return v._helpers.format(v.settings.messages.RANGE, min, max);
      return null;
    }
    /**
     * 日付
    ...
    static checkbox(field, nodeList, params, v) {
      ...
      if (2 <= params.length) {
        ...
        if (check < min || max < check)
          return v._helpers.format(v.settings.messages.CHECKBOX_RANGE, min, max);
      } else {
        if (check < min)
          return v._helpers.format(v.settings.messages.CHECKBOX_MIN, min);
      }
    }
    /**
     * 正規表現チェック
    ...
    static regexp(field, nodeList, params, v) {
      ...
      try {
        ...
        if (!reg.test(val))
          return (err_message ? err_message : v.settings.messages.REGEXP_INVALID_VALUE);
      } catch (e) {
